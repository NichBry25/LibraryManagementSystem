...
export async function GET(
  req: Request,
  { params }: { params: Promise<{ member_borrowing : string | string[] }> }
) {
  const { member_borrowing } = await params;

  const idParam = Array.isArray(member_borrowing) ? member_borrowing : member_borrowing;
